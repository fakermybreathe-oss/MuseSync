...
export const PlaylistPanel: React.FC<PlaylistPanelProps> = ({
  ...
  const showLiftedCard = useCallback((cardId: string, sourceElement?: HTMLElement) => {
    ...
    if (sourceElement) {
      ...
      if (stageRect) {
        ...
        const previewHeight = compact ? 118 : 140;
        const previewLeft = compact ? 4 : -268;
        const previewWidth = Math.min(compact ? 342 : 520, stageRect.width + Math.abs(previewLeft) - 4);
        ...
        const clampedScale = Math.max(compact ? 0.64 : 0.52, Math.min(compact ? 0.84 : 0.7, scaleFromSource));
        nextMotion = {
          top,
  ...
  const resetLiftPreviewPointer = useCallback((target: HTMLElement) => {
    ...
    target.style.setProperty('--lift-glint-tx', '0px');
    target.style.setProperty('--lift-glint-ty', '0px');
  }, []);
  ...
  const handleLiftPreviewPointerMove = useCallback((event: React.PointerEvent<HTMLButtonElement>) => {
    ...
    liftPointerStateRef.current = {
      ...
      glintTx: `${(x * 34).toFixed(1)}px`,
      glintTy: `${(y * 22).toFixed(1)}px`,
    };
    ...
    liftPointerFrameRef.current = window.requestAnimationFrame(() => {
      ...
      next.target.style.setProperty('--lift-glint-tx', next.glintTx);
      next.target.style.setProperty('--lift-glint-ty', next.glintTy);
    });
  }, []);
    ...
    <>
      ...
      <style>{`
        ...
        .playlist-stage {
          ...
          --playlist-card-overhang: 292px;
        }

        .playlist-stage::before {
          content: '';
          position: absolute;
          inset: -8px -12px -8px -314px;
          z-index: 5;
          pointer-events: none;
          opacity: 0;
          background:
        ...
        .playlist-track-stack {
          ...
          isolation: isolate;
          transition:
            transform 420ms cubic-bezier(0.16, 1, 0.22, 1),
            opacity 220ms ease;
        ...
        .playlist-stage.has-lift-preview .playlist-track-stack.has-lifted-card {
          transform: translate3d(78px, 6px, -196px) rotateY(-16deg) scale(0.82);
          opacity: 0.38;
          filter: none;
        }

        .playlist-folder-stack.has-lifted-card .playlist-folder-card,
        .playlist-track-stack.has-lifted-card .playlist-track-card {
          animation: none;
        }
        ...
        .playlist-track-stack.has-lifted-card .playlist-track-card:not(.is-lifted) {
          opacity: 0.28;
          transform: translate3d(54px, 0, -156px) rotateY(-13deg) scale(0.84);
        }
        ...
        .playlist-track-card.is-lifted {
          opacity: 0.08;
          transform: translate3d(48px, 0, -166px) rotateY(-14deg) scale(0.84);
        ...
        .playlist-lift-preview {
          position: absolute;
          left: -268px;
          width: min(520px, calc(100% + 264px));
          height: 140px;
          --lift-tilt-x: 0deg;
          --lift-tilt-y: 0deg;
          --lift-glint-tx: 0px;
          ...
          transform: translate3d(var(--lift-source-x), var(--lift-source-y), -108px) rotateX(8deg) rotateY(-24deg) scale(var(--lift-source-scale));
          transform-style: preserve-3d;
          transition:
            opacity 120ms ease,
            transform 380ms cubic-bezier(0.16, 1, 0.18, 1);
          box-shadow: none;
          will-change: transform, opacity;
          backface-visibility: hidden;
        ...
        .playlist-lift-preview.is-visible {
          opacity: 1;
          pointer-events: auto;
          transform: translate3d(-96px, -8px, 238px) rotateX(1deg) rotateY(-8deg) scale(1.018);
        }

        .playlist-lift-preview.is-leaving {
          opacity: 0;
          pointer-events: none;
          transform: translate3d(var(--lift-source-x), var(--lift-source-y), -92px) rotateX(8deg) rotateY(-22deg) scale(var(--lift-source-scale));
        }
        ...
        .playlist-lift-inner {
          ...
          transform-origin: 96% 52%;
          opacity: 0.86;
          transform: translate3d(0, 0, 0) rotateX(2deg) rotateY(-4deg) scale(0.9);
          transition:
            opacity 220ms ease,
            transform 360ms cubic-bezier(0.16, 1, 0.18, 1);
          will-change: transform, opacity;
        }

        .playlist-lift-preview.is-visible .playlist-lift-inner {
          opacity: 1;
          transform: translate3d(0, 0, 0) rotateX(var(--lift-tilt-x)) rotateY(var(--lift-tilt-y)) scale(1);
          transition:
            opacity 120ms ease,
        ...
        .playlist-lift-preview::after {
          ...
          z-index: -1;
          pointer-events: none;
          background: radial-gradient(ellipse at center, rgba(68, 158, 176, 0.08), rgba(68, 158, 176, 0));
        ...
        .playlist-lift-backplate {
          ...
          border-radius: 30px;
          background:
            linear-gradient(135deg, rgba(248,252,252,0.38), rgba(232,246,248,0.16)),
            radial-gradient(circle at 18% 18%, rgba(34,211,238,0.18), transparent 34%),
            radial-gradient(circle at 96% 88%, rgba(217,119,6,0.12), transparent 35%),
            rgba(210, 235, 238, 0.2);
          box-shadow:
            0 14px 30px rgba(28, 91, 105, 0.12),
          ...
          overflow: hidden;
          contain: paint;
          transform: translate3d(0, 0, 14px) scale(0.9);
        ...
        .playlist-lift-backplate::before {
          ...
          pointer-events: none;
          background:
            radial-gradient(circle at 50% 50%, rgba(255,255,255,0.48), rgba(255,255,255,0.12) 25%, transparent 48%),
            linear-gradient(115deg, rgba(255,255,255,0.18), transparent 38%, rgba(255,255,255,0.08) 68%, transparent);
          opacity: 0.8;
          transform: translate3d(var(--lift-glint-tx), var(--lift-glint-ty), 0);
        ...
        .playlist-lift-preview.is-visible .playlist-lift-backplate {
          opacity: 1;
          transform: translate3d(0, 0, 44px) scale(1);
        }
        ...
        .playlist-lift-cover-wrap {
          ...
          overflow: hidden;
          box-shadow:
            0 14px 28px rgba(28, 91, 105, 0.18),
            0 0 0 1px rgba(255,255,255,0.28),
            0 0 22px rgba(255,255,255,0.12),
            inset 0 1px 1px rgba(255,255,255,0.32);
          transform: translate3d(18px, 0, 20px) scale(0.84);
          transition:
            transform 360ms cubic-bezier(0.16, 1, 0.18, 1),
            border-radius 340ms cubic-bezier(0.2, 1.06, 0.22, 1);
          transform-style: preserve-3d;
          will-change: transform;
        }
        ...
        .playlist-lift-preview.is-visible .playlist-lift-cover-wrap {
          border-radius: 24px;
          transform: translate3d(-4px, -2px, 82px) scale(1.04);
        }
        ...
        .playlist-lift-copy {
          ...
          padding-top: 1px;
          opacity: 0.82;
          transform: translate3d(20px, 0, 18px) scale(0.9);
          transition:
            opacity 260ms ease,
            transform 360ms cubic-bezier(0.16, 1, 0.18, 1);
          will-change: transform, opacity;
        ...
        .playlist-lift-title {
          ...
          text-overflow: ellipsis;
          white-space: nowrap;
          text-shadow: 0 3px 14px rgba(28, 91, 105, 0.42), 0 1px 2px rgba(8, 18, 22, 0.18);
        }
        ...
        .playlist-lift-actions {
          ...
          transition:
            opacity 260ms ease,
            transform 340ms cubic-bezier(0.16, 1, 0.18, 1);
        }
        ...
        @media (max-width: 768px) {
          ...
          .playlist-lift-backplate {
            left: 0;
            border-radius: 23px;
            transform: translate3d(0, 0, 14px) scale(0.985);
          }
          ...
          .playlist-lift-cover-wrap {
            ...
            height: 104px;
            border-radius: 19px;
            transform: translate3d(12px, 0, 18px) scale(0.88);
          }

          .playlist-lift-preview.is-visible .playlist-lift-cover-wrap {
            border-radius: 21px;
            transform: translate3d(-2px, -1px, 62px) scale(1.03);
          }
          ...
          .playlist-lift-copy {
            left: 126px;
            right: 14px;
            transform: translate3d(12px, 0, 16px) scale(0.92);
          }
