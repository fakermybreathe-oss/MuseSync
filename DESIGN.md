...
## 🎨 UI 语义化设计系统 (UI Semantic Design System)

本章节根据 `taste-design` 规范制定，为 MuseSync 所有页面与组件的生成提供唯一的设计事实源。

### 1. 视觉风格与氛围 (Visual Theme & Atmosphere)
*   **设计基调**：Fluid & Pristine Liquid Glass (流动且纯净的液态玻璃)。
*   **氛围描述**：采用不对称的自由空间布局与高透晶莹的液态亚克力材质。整体氛围明亮、清澈且带有温和的日落海滩微光。彻底杜绝廉价感强烈的 neon 外发光圈与毛糙模糊的厚重毛玻璃，以 1px 高透白折射线（Whisper Border）与高透光率卡片为核心，传达极具物理质感与三维纵深的空间艺术。
*   **设计维度**：
    *   **密度 (Density)**：日常应用平衡（5/10）
    *   ** variance (Variance)**：非对称偏移（7/10）
    *   **动效 (Motion)**：沉稳物理阻尼（7/10）

### 2. 色彩系统与角色 (Color Palette & Roles)
*   **日落海滩底色 (Canvas Tide)** (背景图) — 温暖明亮的日落海滨图，叠加上空缓慢漂流的晚霞偏光。
*   **高透液态面板 (Liquid Glass Panel)** (`rgba(5, 16, 23, 0.46)`) — 高透明度、低模糊度的卡片填充，完美保留背景明亮感。
*   **纯净文本 (Pristine Ink)** (`rgba(255, 255, 255, 0.95)`) — 主要标题与文字，确保极致对比与清晰度。
*   **哑光白钢 (Muted Alabaster)** (`rgba(255, 255, 255, 0.5)`) — 次要说明文字、标签与辅助元数据。
*   **折射边界 (Whisper Border)** (`rgba(255, 255, 255, 0.18)`) — 卡片边缘、按钮边框，用于呈现 1px 精细折光层。
*   **落日琥珀 (Sunset Amber)** (`#D97706`) — 独家点缀色，仅用于主行动按钮、聚焦输入框边框及重点高亮，饱和度严格限制在 80% 以下。

### 3. 字体排版规范 (Typography Rules)
*   **Display 标题**：`Outfit` 字体。字距必须紧缩（`letter-spacing: -0.035em`），字重设为 `800 (ExtraBold)`。通过紧密的字距和强对比的字重传达画廊般的高级感，禁止使用过大字号或刺眼强光。
*   **Body 正文**：`Outfit` 字体。行高设为宽松的 `1.5`，每行最大字符限制在 65ch 内，使用哑光白钢色柔和呈现。
*   **Mono 编码/数字**：`JetBrains Mono` 字体。用于计时器、房间延迟参数等高密度数据。
*   **字体禁忌**：禁止在 UI 中使用 generic system fonts 或 `Inter`；严禁在 dashboard 或播放界面使用 generic serif 衬线字体；全界面**绝对禁用**任何 Unicode 级 Emoji 符号，必须以微型 SVG 矢量图形或精炼中文文本表达。

### 4. 组件精细样式 (Component Stylings)
*   **液态按钮 (Liquid Buttons)**：采用高透玻璃质地基座，搭配 1px 细线描边。点击时触发 -1px 垂直位移与物理挤压形变（`scaleX(1.035) scaleY(0.92)`），松开时伴有阻尼回弹。禁用外发光。
*   **流体输入框 (Fluid Inputs)**：标签严格置于输入框上方，辅以小字号哑光白钢色。输入框主体呈暗透基底，聚焦时去除任何发光模糊底，仅保留 Sunset Amber 细线边框，且光标采用琥珀偏光。
*   **晶莹背景气泡 (Glass Bubbles)**：非交互的 3D 浮动气泡，拥有极强烈的内阴影偏光折射（`inset 0 3px 6px rgba(255, 255, 255, 0.45)`），在海滩背景上缓慢做正弦游摆，构造物理视差。

### 5. 空间布局原则 (Layout Principles)
*   **空间隔离**：每一个元素占据明确的空间，严禁出现绝对定位导致的无序重叠或文字压图。
*   **非对称偏置**：登录面板在桌面端居中，但内部元素排布采用非对称左对齐或错落排布，避免呆板对称。
*   **移动端降级**：当屏幕宽度小于 768px 时，所有多列网格必须无缝收缩为单列，且文字字号通过 `clamp()` 自适应缩小，绝不允许出现横向滚动条。

### 6. 物理动效哲学 (Motion & Interaction)
*   **沉稳阻尼**：GSAP 弹簧物理（Spring Physics）默认参数调校为 `stiffness: 100, damping: 20`，避免轻浮的多次往复震荡（按钮回弹从 `back.out(2.5)` 降为更具重力感的 `back.out(1.2)`）。
*   **瀑布流进场**：组件挂载时，子元素严禁瞬间生硬出现，必须使用 GSAP 实施交错时间轴（Stagger）渐显淡入。
*   **性能保障**：动效必须 100% 运行在 GPU 硬件加速的 `transform` 和 `opacity` 属性上，绝对禁止对 `top`/`left`/`width`/`height` 进行过渡。

### 7. 设计禁忌反模式 (Anti-Patterns - Banned)
*   ❌ **禁止使用 Emoji 表情符号**（如 🔍、🔒、✉️ 等）。
*   ❌ **禁止使用 Inter 字体**。
*   ❌ **禁止使用纯黑 (#000000) 背景**。
*   ❌ **禁止使用霓虹外发光 (Neon Glow / Outer Glow)**。
*   ❌ **禁止使用虚假数据或造假统计指标**。
*   ❌ **禁止使用人工智能文案套话**（如 "Seamless", "Elevate", "Next-Gen" 等）。
